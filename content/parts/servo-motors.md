...
The servo motor consists of a motor, gearbox, controller, and [potentiometer](/parts/potentiometers/) in one convenient package. The Arduino sends the servo timed pulses of electricity (called Pulse Width Modulation or PWM). The time between each pulse tells the controller in the servo what angle to rotate to.
...
The servo controller knows what angle it is at with the [potentiometer](/parts/potentiometers/) built into the servo. Because of the potentiometer, the servo has a limited range of motion but is very accurate at moving to exact positions.
